...
class UiManager():
    def __init__(self) -> None:
        self.font = pygame.font.Font(build_path(
            "assets/fonts/AtariClassic.ttf"), FONT_SIZE)
        self.empty_heart = scale_img(pygame.image.load(build_path(
            "assets/images/items/heart_empty.png")).convert_alpha(), ITEM_SCALE)
        self.half_heart = scale_img(pygame.image.load(build_path(
            "assets/images/items/heart_half.png")).convert_alpha(), ITEM_SCALE)
        self.full_heart = scale_img(pygame.image.load(build_path(
            "assets/images/items/heart_full.png")).convert_alpha(), ITEM_SCALE)
        self.coin = scale_img(pygame.image.load(build_path(
            "assets/images/items/coin_f0.png")).convert_alpha(), ITEM_SCALE)
        self.start_button = Button(
            SCREEN_WIDTH // 2 - 145, SCREEN_HEIGHT // 2 - 150, "assets/images/buttons/button_start.png")
        self.exit_button = Button(
            SCREEN_WIDTH // 2 - 110, SCREEN_HEIGHT // 2 + 50, "assets/images/buttons/button_exit.png")
        self.resume_button = Button(
            SCREEN_WIDTH // 2 - 175, SCREEN_HEIGHT // 2 - 150, "assets/images/buttons/button_resume.png")
        self.restart_button = Button(
            SCREEN_WIDTH // 2 - 175, SCREEN_HEIGHT // 2 - 50, "assets/images/buttons/button_restart.png")
    ...
    def draw_ui(self, surface, current_level, player_health, player_score):
        if SHOW_GRID:
            for x in range(30):
                pygame.draw.line(surface, WHITE, (x * TILE_SIZE, 0),
                                 (x * TILE_SIZE, SCREEN_HEIGHT))
                pygame.draw.line(surface, WHITE, (0, x * TILE_SIZE),
        ...
        self.draw_text(
            surface, f"LEVEL: {str(current_level)}", WHITE, SCREEN_WIDTH / 2, 15)
        self.draw_text(surface, f"X{player_score}",
                       WHITE, SCREEN_WIDTH - 100, 15)
